...
that serves `/chat` — there's no separate embedding API or managed vector
service in the loop. This is a real deployment decision, not just a
diagram: it's what keeps the system free to run, at the cost of the
service's own memory budget having to fit both the embedder and
everything else Render's free tier allows (512MB total) — the reason
this project has now gone through two rounds of memory-driven changes
(Checkpoint 4e, then 4f) to actually fit inside it.
